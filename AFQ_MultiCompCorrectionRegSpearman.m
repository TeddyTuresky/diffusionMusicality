function [alphaFWE, statFWE, clusterFWE, stats] = AFQ_MultiCompCorrectionRegSpearman(data,y,alpha,method,cThresh,reg)
        ...
        switch(stattest)
            case('corr')
                for ii = 1:nperm
                    ...
                    [stat(ii,:) p(ii,:)] = partialcorr(y, data(rows1,:), reg(rows2,:), 'rows', 'pairwise','type','Spearman'); % TKT added, 2020
                end
            case('ttest')
                for ii = 1:nperm
                    rows = Shuffle(y);
